...
def agrupar_pago(pago):
    grupos = {
        'Credito_Financ_Externa': [
            'Crédito ICETEX', 'Financ por Banco o Cooperativa', 'Crédito Coop. Comuna'
        ],
        'Pago_Directo_Propio': [
            'Contado / Efectivo', 'Tarjeta de Crédito', 'Fondo de Cesantias'
        ],
        'Apoyo_Beneficio': [
            'Beca', 'Subsidio Empresarial'
        ],
        "Otro": ["Otro"]
    }
    for grupo, valores in grupos.items():
...
# ------------------------------
st.set_page_config(page_title="Prototipo Multimodelo", page_icon="🎓", layout="wide")
...
# ------------------------------
# Entrada de datos en 3 columnas
# ------------------------------
...
if st.button("🔮 Predecir"):
    try:
        ...
        with c3:
            ...
            ax.set_title("Distribución", fontsize=8)
            st.pyplot(fig)
        ...
        if prob >= 0.7:
            ...
            st.warning("El modelo estima una **probabilidad intermedia**. Recomendable seguimiento al aspirante.")
        else:
            st.error("El modelo estima una **baja probabilidad** de matrícula. Requiere estrategias adicionales.")
